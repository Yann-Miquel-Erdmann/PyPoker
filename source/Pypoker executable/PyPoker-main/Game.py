...
class Game():
    ...
    def __init__(self, app, images, skins, utilisateur,):
        ...
        self.var = IntVar()
        self.joueurs = [Player(argent=self.montant_de_depart) for i in range(2)]
        ...
        for _ in range(2):  # donne deux self.cartes à chacun
            for i in range(len(self.joueurs)):
                self.joueurs[i].givecards(self.jeudecartes.pop())
        ...
        self.cartesframes = []
        pos = [185, 290, 600, 705]
        for i in range(4):  # cree les 4 cartes avec leur fonctions click et release
            self.cartesframes.append(Frame(self.frame_jeu, bg="yellow"))
            self.cartesframes[-1].place(x=pos[i], y=450)
            
            self.cartes.append(Label(self.cartesframes[-1], image=self.skins[self.skin_selectionne+8], cursor="hand2"))
            self.cartes[-1].pack()
            self.cartes[i].bind(
                "<Button-1>", 
                lambda event, a=0 if i < 2 else 1: self.interactCarte(a, "click")
            )
            self.cartes[i].bind(
                "<ButtonRelease-1>",
                lambda event, a=0 if i < 2 else 1: self.interactCarte(a, "release"),
            )
        ...
        self.boutonAccueil.bind("<Button-1>", lambda event: self.app.master.children["!frame"].pack(expand=True)or self.app.pack_forget())
        ...
        for i in range(2):
            for j, bouton in enumerate(self.boutons.keys()):
                ...
                self.boutons[bouton][i].place(x=xpos[i], y=ypos[j])
                self.boutons[bouton][i].bind("<Button-1>", lambda event, a=i,
                                        btn=bouton: self.boutonInteract(event, a, btn, "click"),)
                self.boutons[bouton][i].bind(
                    "<ButtonRelease-1>",
                    lambda event, a=i, btn=bouton: self.boutonInteract(
                        event, a, btn, "release"),
                )
        ...
        self.labels = []
        for i in range(2):
            self.labels.append(Label(self.frame_jeu, text="argent: "+str(self.joueurs[i].get_argent()),width=19, bg="#222222", fg="white"))
            self.labels[i].place(x=xpos[i], y=370)

        pos = [290, 600]
        self.mises = []
        for i in range(2):
            self.mises.append(Label(self.frame_jeu, text="0", bg="#222222", fg="white"))
            self.mises[i].place(x=pos[i], y=420, width=90)

        pos = [185, 705]
        self.entry = []
        for i in range(2):
            self.entry.append(Entry(self.frame_jeu, fg="gray"))
            self.entry[i].place(x=pos[i], y=400, height=40, width=90)
            self.entry[i].bind(
                "<KeyRelease>", lambda event, i=i: self.OnKeyPress(event, i)
            )  # on detecte l'écriture
            self.entry[i].bind(
                "<FocusIn>", lambda event: self.focus_in(event)
            )  # on détecte l'entrée dans la zone de texte
            self.entry[i].bind("<FocusOut>", lambda event: self.focus_out(event))
            self.entry[i].insert(0, "Montant")
            ToolTip(self.entry[i], "Entrez le montant de votre mise ici.")   # !  ToolTip


        self.rivierelabel = []
        for i in range(6):  
            self.rivierelabel.append(Label(self.frame_jeu, image=self.skins[self.skin_selectionne+8], cursor="hand2"))
    ...
    def newround(self):
        ...
        self.skin_selectionne = self.utilisateur.skin_selectionne
        for carte in self.cartes:
            carte.configure(image=self.skins[self.skin_selectionne+8])
        
        for label in self.rivierelabel:
            label.configure(image=self.skins[self.skin_selectionne+8])
    ...
    def allin(self,player):
        ...
        if self.lastmise >= valeur:
            ...
            self.endriver()
        elif self.joueurs[int(not player)].get_argent() == 0:  #mise > allin de l'autre joueur
                self.endriver()
        else:
    ...
    def mise(self,player, valeur):
        ...
        if self.joueurs[player].miser(valeur) == True:
            ...
            if valeur == self.lastmise:  
                ...
                self.endriver()
            else:  # c'est au joueur suivant de jouer
                self.lastmise = valeur
                self.joueuractif = 0 if self.joueuractif else 1
                self.showtour()
                self.joueur_tour_suivant = player
    ...
    def suivrecall(self,player):
        ...
        if self.lastmise == -1: #si parole des le premier tour
            self.lastmise = 0
            self.joueuractif = 0 if self.joueuractif else 1
            self.showtour()
    ...
    def gagnantjoueur(self,):
        ...
        if self.joueurs[gagnant].get_argent() == 2*self.montant_de_depart:
            self.winner_text.set(f"Le joueur {gagnant+1} gagne la partie")
    ...
    def end_game_bouton(self,event, action, name):
        if action == "click":
            ...
            if name == "rejouer":
                ...
                self.app.master.children["!frame"].pack(expand=True)
            self.frame_jeu.pack(expand=True)
            
            self.frame_end.pack_forget()


    def boutonInteract(self,event, player, button, action):
        if action == "click":
            if button == "btn_parole":
                self.suivrecall(player=player)

            elif button == "btn_miser":
                Jouer_Son_async("Miser")
                val = self.entry[player].get()
                if len(val) > 0:
                    montant = int(val)
                    self.mise(player=player, valeur=montant)
                else:
                    print("veuilez entrer le montant de votre mise")

            elif button == "btn_secoucher":
                self.secouche(player=player)
            elif button == "btn_tapis":
                Jouer_Son_async("Miser")
                self.allin(player=player)
            ...
            self.boutons[button][player].configure(image=self.images[newbutton + "_click"])

        else:

            self.boutons[button][player].configure(image=self.images[button])
            self.update_parole_relance()
    ...
    def update_parole_relance(self,):
        ...
        if self.suivre:
            for i in range(2):
                self.boutons["btn_parole"][i].config(image=self.images["btn_suivre"])

        else:
            for i in range(2):
                self.boutons["btn_parole"][i].config(image=self.images["btn_parole"])

        if self.relance:
            for i in range(2):
                self.boutons["btn_miser"][i].config(image=self.images["btn_relancer"])
        else:
            for i in range(2):
                self.boutons["btn_miser"][i].config(image=self.images["btn_miser"])
    ...
    def interactCarte(self,a, action):
        ...
        if len(self.joueurs):
            ...
            if action == "click":
                ...
                self.cartes[i+1].image = self.joueurs[a].get_cartes_images()[0]

            elif action == "release":
                self.cartes[i].configure(image=self.skins[self.skin_selectionne+8])
                self.cartes[i + 1].configure(image=self.skins[self.skin_selectionne+8])
    ...
    def OnKeyPress(self,event, player):  # si on écrit dans l'element de texte
        ...
        if not value.isnumeric():
            event.widget.delete(0, END)
            event.widget.insert(0, ''.join([c for c in value if c.isnumeric()]))
        elif int(event.widget.get()) > self.joueurs[player].get_argent():
                event.widget.delete(0, END)
